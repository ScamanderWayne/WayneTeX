...
\lsection{Pojištění \CHECK}
\qaw{Co je pojištění}{%
Zvláštní odvětví ekonomiky, jedná se o finanční službu, neznamená růst bezpečí ani naprostou jistotu.
}
...
Nejčastěji se jedná o akciovou společnost, někdy i o družstevní organizace.
Licence spravuje ČNB.
}
\qaw{Vysvětli:}{%
\textbl{pojistník --}
fyzická nebo právnická osoba, co uzavřela pojistnou smlouvy (rodič na dítě, osoba na auto)\\
\textbl{pojištěný --}
...
\begin{itemize}[label={\color{WPP}\faSlackHash}]
\item \textsc{\color{WPP} Zákonné pojištění --} je automaticky uzavřeno dle zákona, pro všechny platí stejné podmínky, je povinné (pojištění zodpovědnosti za provoz motorových vozidel, ZP, SP)\\
...
\item \textsc{\color{WPP} Povinně smluvní pojištění --} týká se oblastí, kde je zvýšené nebezpečí ohrožení ostatních, patří sem povinné ručení
\item \textsc{\color{WPP} Smluvní pojištění --} na základě smlouvy a je dobrovolné, je ovlivněno všeobecnými a zvláštními podmínkami, jednotlivé pojišťovny mohou mít ještě vlastní specifika
\end{itemize}
}
\qaw{Podle formy tvorby rezerv}{%
\begin{itemize}[label={\color{WPP}\faSlackHash}]
\item \textsc{\color{WPP} Rizikové pojištění --} neví se kdy nebo jestli pojistná událost vznikne (úrazové, majetkové, ...)
\item \textsc{\color{WPP} Rezervotvorné pojištění --} ví se, kdy vznikne (životní a důchodové)
\end{itemize}
}
\qaw{Podle odvětví}{%
\begin{itemize}[label={\color{WPP}\faSlackHash}]
\item \textsc{\color{WPP} Neživotní -- }na majetek
\item \textsc{\color{WPP} Životní --} na osobu
\end{itemize}
}
...
Systém bonusů a přirážek používaný u havarijného pojištění. Za období beznehody dostáváte bonusy, odečty od ceny pojištění. Za nehody se tyto bonusy odečítají až do záporu, kdy naopak vzniká riziková přirážka, tedy malus.
}
