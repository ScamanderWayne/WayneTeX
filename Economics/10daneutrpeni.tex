...
-- daňové přiznání se podává do 31. 3. dalšího roku, pokud má plátce daňového poradce, tak až do 30. 6.\\
...
-- předmětem daně nejsou příjmy z dědictví, darů, úvěrů a půjček; osvobozeny od daní jsou příjmy z nemocenského pojištění, sociálních dávek a movitých věcí; prodej nemovitostí je osvobozen, pokuj je vlastníme déle než 2 roky (nebo peníze použijeme na nákup jiného bydlení)\\
\textbl{kdy mám povinnost uvést příjem nad pět milionů}
-- vždy, i když je vyjmut z daňové povinnosti, tak se musí uvést (prodej domu, výhra v loterii, kterou již zdanil provozovatel)\\
\textbl{slevy}
-- na poplatníka; na manželku (pod hranicí aktuálního příjmu); na děti; ZTP\\
\textbl{úlevy}
-- úlevy mohou vzniknout: z úroků z hypotečního úroku; z dárcovství krve a plasmy (pokud byla poskytnuta bez odměny); z poskytnutí darů za určitých podmínek
}
\qaw{Jaké možnosti podání daňového přiznání má OSVČ}{%
OSVČ musí podávat přiznání přes Datovou Schránku, má k tomu tři možnosti.\\
\begin{itemize}[label={\color{WPP}\faSlackHash}]
\item \textbl{paušální daň} -- pro neplátce DPH, jedná se o kombinaci ZP, SP a daně z příjmů, vhodná pro menší podnikatele
\item \textbl{výdajový paušál} -- pro základ daně použije část svých příjmů (20 \% -- 80 \% dle sektoru), nemusí tedy dokládat reálné výdaje, což zjednodušuje přiznání
\item \textbl{sledování příjmů a výdajů} -- OSVČ, který vede daňovou evidenci si spočítá daňový základ jako rozdíl mezi výdaji a příjmy, musí tedy řádně vést administrativu, ale platí jen to, co reálně musí
\end{itemize}
}
\qaw{Za jakých podmínek musím platit 23 \% daň z příjmu}{%
Pokud daňový základ přesuhuje 36násobek průměrné mzdy (přibližně 1 800 000 Kč), kdy se 23 \% platí jen u příjmů, co hranici přesahuje.
}
\newpage
\qaw{Daň z nemovitosti}{%
\textbl{termín podání přiznání}
-- přiznání se podává k 31. 1. roku, ve kterém vznika daňová povinnost; splatnost pak je k 31. 5. stejného roku\\
...
-- nemovitosti, jakou jsou pozemky (stavební prostory, lesy, louky, vodní plochy), stavby (prostory k bydlení, nebytové prostory, ale i stodoly a další), bytové jednotky tvoří samostatnou kategorii;\\
předmětem daně jsou jen nemovitosti zapsané v katastru k 1. 1.;\\
výjimku z daně mají památky a nemovitosti sloužící veřejnému zájmu (jako jsou školy a nemocnice)\\
\textbl{kam plyne tato daň}
-- celá částka daně plyne přímo do rozpočtů měst a obcí, kde se nemovitost nachází
}
\qaw{Silniční daň}{%
\textbl{kdy se platí}
-- musí jí platit vlastníci nebo provozovatelé vozidel, co mají vozidlo zapsané k obchodní činnosti a vozidlo přesahuje hmotnost 12 tun;\\
...
\begin{tikzpicture}[scale=\tikzscale, snake=zigzag, line before snake = 5mm, line after snake = 5mm]
    ...
    \foreach \x in {0,6,12,15}
      ...
      \foreach \x in {0.5,4.5,5,2.5,6.5,7.5,8.5,9}
      \draw[ultra thick, WPP] (\x cm,3pt) -- (\x cm,-3pt);

    % draw nodes
    \draw (0,0) node[below=7pt] {} node[above=7pt] {2024};
    
    \draw (0.5,0) node[below=4pt] {\begin{varwidth}{2cm}\scriptsize k \texthl{31. 1.} povinost podat přiznání k \emph{dani z~nemovitosti}\end{varwidth}} node[above=4pt] {};
    \draw[WPP, ultra thick] (4.5,0) -- (5,0) node[below=4pt] {} node[above=4pt] {};
    \draw (4.7,0) node[below=4pt] {} node[above=4pt] {\begin{varwidth}{5cm}\scriptsize splatné měsíčně\end{varwidth}};
	\draw[antiDP, thick] (3,0) node[below=4pt] {\begin{varwidth}{2.5cm}\scriptsize k \texthl{31. 5.} splatnost \emph{daně z~nemovitosti}\end{varwidth}} node[above=4pt] {};
  
    \draw (6,0) node[below=7pt] {} node[above=7pt] {2025};
    
    \draw[antiDP, thick] (7.24,0) node[below=4pt] {} node[above=4pt] {\begin{varwidth}{2cm}\scriptsize k \texthl{31. 1.} \emph{splatná silniční daň} \end{varwidth}};
    \draw[antiDP, thick] (6.8,0) node[below=4pt] {\begin{varwidth}{5cm}\scriptsize do {\color{WPP}31. 3.} \emph{splatná daň z příjmů}\end{varwidth}} node[above=4pt] {};
    \draw[antiDP, thick] (8.5,0) node[below=4pt] {} node[above=4pt] {\scriptsize květen};
    \draw[antiDP, thick] (10.2,0) node[below=4pt] {\begin{varwidth}{4cm}\scriptsize do {\color{WPP}30. 6.} \emph{splatná daň z příjmů} pokud ji zpracovává {\color{WPP}daňový poradce}\end{varwidth}} node[above=4pt] {};
    
    \draw (12,0) node[below=7pt] {} node[above=7pt] {2026};
    \draw (15,0) node[below=7pt] {} node[above=7pt] {2027};
